  ...
  return (
    <div style={{minHeight:'100vh',background:'#FAFAF7',fontFamily:'sans-serif',direction:'rtl'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',background:'#F2F1EC',borderBottom:'1px solid #E5E4DE'}}>
        {[
          {num:'142', label:'ريال مكتسب', color:'#10B981'},
          {num:'13', label:'حضور'},
          {num:'94%', label:'قبول'},
        ].map((s,i) => (
          <div key={i} style={{padding:'14px 8px',textAlign:'center',borderLeft: i>0 ? '1px solid #E5E4DE' : 'none'}}>
            <div style={{fontSize:20,fontWeight:600,letterSpacing:'-0.02em',color:s.color||'#0A0A0A',marginBottom:2}}>{s.num}</div>
      ...
      <div style={{padding:'0 16px',paddingBottom:40}}>
        {[
          {section:'الحساب', items:[
            {icon:'👤', label:'تعديل الملف الشخصي'},
            {icon:'🏦', label:'بيانات التحويل البنكي', action:'/wallet'},
            {icon:'🔔', label:'الإشعارات'},
          ]},
          {section:'الأداء', items:[
            {icon:'📊', label:'إحصائياتي', action:'/wallet'},
            {icon:'🎯', label:'فعالياتي', action:'/'},
          ]},
          {section:'أخرى', items:[
            {icon:'❓', label:'مركز المساعدة'},
            {icon:'📋', label:'الشروط والخصوصية'},
            {icon:'🚪', label:'تسجيل الخروج', color:'#EF4444', action:'/auth'},
          ]},
        ].map((section,si) => (
            ...
            <div style={{background:'white',border:'1px solid #E5E4DE',borderRadius:12,overflow:'hidden'}}>
              {section.items.map((item,ii) => (
                <div key={ii}
                  ...
                  <span style={{flex:1,fontSize:14,fontWeight:500,color:item.color||'#0A0A0A'}}>{item.label}</span>
