...
export default function MerchantDashboard() {
  ...
  const [stats, setStats] = useState({ totalAttendees: 0, activeEvents: 0, totalPoints: 0 })
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (eventsData) {
        ...
        const activeCount = eventsData.filter((e: any) => e.status === 'published').length
        const totalAtt = eventsData.reduce((sum: number, e: any) => sum + (e.attendees?.[0]?.count || 0), 0)
        setStats({ totalAttendees: totalAtt, activeEvents: activeCount, totalPoints: 0 })
      }
    }
    fetchData()
  }, [])
    ...
    <div className="page page-content" data-page-id="dashboard" id="page-dashboard">
      ...
      <div className="kpi-row">
        <div className="kpi-card">
          ...
          <div className="kpi-value">{stats.totalAttendees.toLocaleString()}</div>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value">{stats.activeEvents}</div>
          <div className="kpi-trend up">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
            +3
